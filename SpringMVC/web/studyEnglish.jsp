...
<form action="readFile" id= tom method=post>
    选择一篇课文：<br>
    <label for="tom"><% File f = new File(".");//文件f被认为在Tomcat安装目录的\bin中。
    ...
    %></label><select id= tom name="fileName" size =1>
    <%  for(int i = 0; i< Objects.requireNonNull(name).length; i++){
    %>    <option selected value='<%=name[i]%>'></option><%=name[i]%>
    <%  }
    %>  </select>
